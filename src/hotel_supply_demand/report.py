...
CSV_FIELDS = [
    "prefecture_code", "prefecture_name", "target_year", "base_year", "release_type",
    "total_guests", "average_occupancy_rate", "average_facilities", "foreign_share_pct",
    "demand_vs_base_pct", "demand_yoy_pct", "occupancy_yoy_pp", "facility_yoy_pct",
    "absorption_gap_pct", "foreign_share_yoy_pp", "monthly_cv", "peak_month_share_pct",
    "market_state", "is_watch", "watch_reasons", "observed_fact", "interpretation",
    "next_action", "data_quality_note",
]
...
.bars{{height:180px;display:flex;align-items:flex-end;gap:5px;border-bottom:1px solid var(--line);padding-top:16px}}.bar{{flex:1;background:#3c8296;min-height:2px;position:relative}}.bar span{{position:absolute;top:100%;width:100%;text-align:center;font-size:.7rem;color:var(--muted);padding-top:3px}}
...
def _monthly_chart(values: list[float]) -> str:
    ...
    bars = "".join(
        f'<div class="bar" style="height:{(value / maximum * 100) if maximum else 0:.1f}%" title="{month}月: {value:,.0f}"><span>{month}</span></div>'
        for month, value in enumerate(values, 1)
    )
    return f'<div class="bars" role="img" aria-label="月別延べ宿泊者数">{bars}</div>'


def _market_sheet(row: dict, as_of: str) -> str:
    reasons = "".join(f"<li>{html.escape(reason)}</li>" for reason in row["watch_reasons"]) or "<li>設定した検知条件への該当なし</li>"
    cards = [
        ("延べ宿泊者数", _fmt(row["total_guests"])),
        (f"{row['base_year']}年比", _fmt(row["demand_vs_base_pct"], "%")),
        ("需要前年比", _fmt(row["demand_yoy_pct"], "%")),
        ("平均客室稼働率", _fmt(row["average_occupancy_rate"], "%")),
        ("稼働率前年差", _fmt(row["occupancy_yoy_pp"], "pt")),
        ("外国人比率", _fmt(row["foreign_share_pct"], "%")),
    ]
...
<p class="sub">対象年 {row['target_year']}年／公表区分 {row['release_type']}／データ取得基準 {html.escape(as_of)}</p>
<p><span class="tag">{html.escape(row['market_state'])}</span></p><div class="cards">{card_html}</div>
<section class="panel"><h2>月別延べ宿泊者数</h2>{_monthly_chart(row['monthly_total_guests'])}<h2>観測事実</h2><p>{html.escape(row['observed_fact'])}</p><h2>検知理由</h2><ul>{reasons}</ul></section>
...
def _index_html(rows: list[dict], config: AnalysisConfig, as_of: str) -> str:
    ...
    table_rows = "".join(
        ...
        f'<td>{html.escape(row["market_state"])}</td><td>{_fmt(row["demand_vs_base_pct"], "%")}</td><td>{_fmt(row["demand_yoy_pct"], "%")}</td>'
        f'<td>{_fmt(row["occupancy_yoy_pp"], "pt")}</td><td>{_fmt(row["facility_yoy_pct"], "%")}</td><td>{html.escape(" / ".join(row["watch_reasons"]) or "—")}</td></tr>'
        for row in rows
    )
    body = f"""<h1>宿泊市場モニタリング</h1><p class="sub">{config.target_year}年確定値／基準年 {config.base_year}年／データ取得基準 {html.escape(as_of)}</p>
...
<h2>都道府県別結果</h2><div class="scroll"><table><thead><tr><th>都道府県</th><th>市場状態</th><th>{config.base_year}年比</th><th>需要前年比</th><th>稼働率前年差</th><th>施設数前年比</th><th>検知理由</th></tr></thead><tbody>{table_rows}</tbody></table></div>"""
