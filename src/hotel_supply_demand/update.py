...
def _write_prefecture_config(
    path: Path, source_page: str, sources: list[Source]
) -> Path:
    temporary = path.with_suffix(path.suffix + ".update.tmp")
    lines = [f"source_page = {_quoted(source_page)}", ""]
    for source in sources:
...
def _merge_municipality_sources(
    ...
    for source in discovered:
        ...
        if previous is None:
            merged[key] = source
            changes.append(
                {"period": f"{source.year}-{source.month:02d}", "kind": "new"}
            )
        elif (
            previous.provider != "estat" and previous.stat_inf_id != source.stat_inf_id
        ):
            approvals.append(
                {
            ...
            merged[key] = source
            changes.append(
                {"period": f"{source.year}-{source.month:02d}", "kind": "revised"}
            )
    return (
...
def update_municipality(
    ...
    *,
    ...
    notify(f"市区町村原Excelの更新を確認しています（{len(selected)}ファイル）")
...
def update_prefecture(
    ...
    *,
    ...
    notify(f"都道府県確定値Excelの更新を確認しています（{file_count}ファイル）")
    fetched = fetch_sources(selected, raw_dir, check_remote=True)
    changed_years = {
        int(item["year"]) for item in fetched if item["status"] == "downloaded"
    }
    if years:
    ...
    with tempfile.TemporaryDirectory(
        ...
        staging = Path(temporary)
        staged_database = staging / database.name
        # build_database preserves municipality tables from its destination.
        # Seed the staged path with the current unified DB before rebuilding
        # prefecture facts, otherwise a prefecture-only update would erase them.
        if database.exists():
            shutil.copy2(database, staged_database)
