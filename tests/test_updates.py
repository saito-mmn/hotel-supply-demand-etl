...
def municipality_source(provider: str = "estat") -> MunicipalitySource:
    if provider == "estat":
        ...
        url = "https://www.e-stat.go.jp/stat-search/file-download?statInfId=000040500001&fileKind=0"
    else:
...
class UpdateTest(unittest.TestCase):
    ...
    def test_estat_parser_accepts_only_annual_final_original_excel(self) -> None:
        ...
        self.assertEqual(
            candidates[0].url,
            "https://www.e-stat.go.jp/stat-search/file-download?statInfId=000040475399&fileKind=0",
        )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                ...
                patch(
                    ...
                    return_value=[{"year": 2026, "month": 6, "status": "unchanged"}],
                ),
            ):
    ...
    def test_load_failure_keeps_database_report_and_config_untouched(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                ...
                patch(
                    "hotel_supply_demand.update.fetch_municipality_sources",
                    return_value=[{"year": 2026, "month": 6, "status": "downloaded"}],
                ),
                patch(
                    "hotel_supply_demand.update.load_municipality_source_records",
                    side_effect=ValueError("invalid workbook"),
                ),
                self.assertRaisesRegex(ValueError, "invalid workbook"),
            ):
                update_municipality(sources_path, root / "raw", database, report)
    ...
    def test_prefecture_update_seeds_staging_with_existing_unified_database(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            database = root / "market.sqlite3"
            database.write_bytes(b"database-with-municipalities")
            report = root / "reports"
            report.mkdir()
            source = Source(
                year=2025,
            ...
            def build(_sources, _raw, staged_database, _progress):
                self.assertEqual(
                    staged_database.read_bytes(), b"database-with-municipalities"
                )
                staged_database.write_bytes(b"rebuilt-unified-database")
                return {"rows": 1}

            def report_result(_database, staged_report, _config):
                staged_report.mkdir()
                (staged_report / "index.html").write_text("new", encoding="utf-8")
                return {"market_sheets": 1}

            with (
                patch("hotel_supply_demand.update.load_sources", return_value=[source]),
                patch(
                    "hotel_supply_demand.update.prefecture_source_page",
                    return_value="https://example.com",
                ),
                patch(
                    "hotel_supply_demand.update.discover_prefecture_sources",
                    return_value=[],
                ),
                patch(
                    "hotel_supply_demand.update.fetch_sources",
                    return_value=[{"year": 2025, "status": "downloaded"}],
                ),
                patch(
                    "hotel_supply_demand.update.build_prefecture_database",
                    side_effect=build,
                ),
                patch(
                    "hotel_supply_demand.update.load_analysis_config",
                    return_value=object(),
                ),
                patch(
                    "hotel_supply_demand.update.generate_reports",
                    side_effect=report_result,
                ),
            ):
                result = update_prefecture(
                    sources_path,
                    root / "raw",
                    database,
                    report,
            ...
            self.assertEqual(database.read_bytes(), b"rebuilt-unified-database")
