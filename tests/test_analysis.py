...
class AnalysisTest(unittest.TestCase):
    def test_metrics_and_reports(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("観光庁「宿泊旅行統計調査」", index_html)
            self.assertIn("e-Stat", index_html)
            self.assertIn("を加工して作成", index_html)
            self.assertIn("観光庁・e-Statが本レポートを作成または内容を保証するものではありません", index_html)
