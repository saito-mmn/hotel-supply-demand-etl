...
Phase 1では、2019～2025年の公式年確定値Excelについて、固定URLからの安全な取得、manifest・SHA-256による来歴管理、共通スキーマへの変換、品質検証、需要・供給SQLiteの再生成まで実装しています。Phase 2では、2019年比・前年比・需要構成・季節性・需要吸収ギャップを計算し、47都道府県のマーケットシートと市場ウォッチリストをCSV・HTMLで生成します。入力Excel、manifest、生成DBは再生成可能なためGit管理しません。
...
| `analysis.py` | DBから年次指標を計算し、設定可能な閾値で市場状態と検知理由を生成する |
| `report.py` | 47都道府県の分析CSV、ウォッチリスト、静的HTMLマーケットシートを生成する |
...
各列の定義、単位、欠測値の扱いは[データ辞書](docs/data-dictionary.md)に記載しています。Phase 2で追加する分析用Viewは永続テーブルと分け、別途データフローとして整理します。
...
2025年確定値の初期設定では20県がウォッチ条件に該当しました。この件数は信用リスクの高い県数ではなく、対象ホテル・商圏の追加確認を促す一次スクリーニング結果です。指標、閾値、分類順序、利用上の制約は[分析方法論](docs/methodology.md)、閾値は[`analysis.toml`](analysis.toml)に明示しています。
