...
| `monthly_demand` | `year + month + prefecture_code + release_type` | `source_file_id` |
| `monthly_supply` | `year + month + prefecture_code + release_type` | `source_file_id` |
...
| View | 役割 |
|---|---|
| `latest_monthly_demand` | 年月・都道府県ごとに`final`を優先した需要レコードを選択 |
| `latest_monthly_supply` | 年月・都道府県ごとに`final`を優先した供給レコードを選択 |
| `monthly_market` | 需要、供給、都道府県名を年月・都道府県で結合 |
| `annual_market` | `monthly_market`を年・都道府県・公表区分で年次集計 |
