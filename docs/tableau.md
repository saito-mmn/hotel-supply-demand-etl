...
## `prefecture_annual_summary.csv`

`prefecture_monthly.csv`が時系列（月次）なのに対し、こちらは**都道府県×完全暦年1行**の横断集計で、地域比較・スクリーニング用途に使う。粒度は`year × prefecture_code`。

12か月分すべてが確定値で揃っている暦年のみを収録する（直近年で確定値がまだ一部の月しか出ていない場合はその年を丸ごと除外する。中途半端な年を「今年の実績」として見せないため）。

主な列：

| 列 | 内容 |
|---|---|
| `guest_nights_total` | その暦年の延べ宿泊者数合計 |
| `foreign_share_pct` | 暦年の外国人延べ宿泊者数 ÷ 総延べ宿泊者数（月次比率の平均ではなく、年間の分子・分母から算出） |
| `occupancy_rate_pct_avg` | 12か月の客室稼働率の単純平均 |
| `occupancy_seasonal_cv` | 客室稼働率の変動係数（標準偏差 ÷ 平均）。**その暦年1年の中の**繁閑の振れ幅であり、複数年にまたがるトレンドやコロナ影響は含まない |
| `occupancy_seasonal_range_pp` | 客室稼働率の年間最高月 − 最低月（pt） |
| `occupancy_peak_month`, `occupancy_bottom_month` | 稼働率が最高・最低だった月（1〜12） |
| `facility_count` | その年12月時点の調査対象施設数 |

季節指標のロジックは`hotel_supply_demand.seasonality.seasonal_profile()`に一本化されていて、静的HTMLレポート（LTM窓）とこのデータセット（暦年窓）の両方から呼ばれる。定義を二重管理しない。
...
3層構成にして、「地域詳細を見る画面」と「全国から気になる地域を発見する画面」を分離する。

```text
① 地域比較（都道府県）        ← prefecture_annual_summary.csv
   外国人比率ランキング、季節変動ランキング、平均稼働率
        ↓ 都道府県を選ぶ
② 都道府県別ホテル需給動向     ← prefecture_monthly.csv
   その県の時系列・需要構成・施設数
        ↓ 市区町村を見る
③ 市区町村別ホテル需給動向     ← municipality_monthly.csv
   ローカル市場の時系列・需要構成・施設数・掲載期間
```

1. **① 地域比較（都道府県）**：`prefecture_annual_summary.csv`を使い、暦年を選んで外国人延べ宿泊者比率・季節変動幅（レンジ優先、CVは補助指標）・平均稼働率でランキング表示する。「外国人依存度」のような断定的な表現は避け、「外国人延べ宿泊者比率」という中立的な値のみ示す。都道府県選択で②へ遷移する導線を持たせる
2. **都道府県市場（②）**：KPI、公式全国値を含む稼働率時系列、日本人・外国人需要構成、日本地図、一覧
3. **市区町村市場（③）**：都道府県・自治体filter、時系列、需要構成、施設数、掲載期間・欠損
4. **施設タイプ別**：優先度を下げて保留。第1・4・8表に施設タイプ（旅館/リゾートホテル/ビジネスホテル/シティホテル/簡易宿所/会社・団体の宿泊所）別の内訳列が既にあることを確認済みだが、着手は地域比較Dashboard完成後
5. **国籍別インバウンド**：現在ダウンロード済みの都道府県年次確定値ワークブックには国籍別の表が存在しない（第9・10表は「県内／県外」であり国籍ではない）。着手には別のe-Stat統計表の新規調査が必要で、優先度は最も低い
