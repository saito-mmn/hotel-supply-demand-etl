...
CIではRuff、テスト、生成HTML検証をプロジェクト全体に適用しています。mypyによる型検査のみ対象を限定しており、現在は更新検出・自動化モジュールを検査します。parser、analysis、report、CLIなど既存モジュールへの拡張は、統合コードレビュー後の品質改善事項です。

## 初回設定

1. GitHubリポジトリの `Settings > Pages > Build and deployment > Source` で `GitHub Actions` を選択する。
2. 必要に応じて `Settings > Environments > github-pages` で、デフォルトブランチだけを許可するdeployment protection ruleを設定する。
