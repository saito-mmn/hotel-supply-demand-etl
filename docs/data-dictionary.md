...
e-Stat「宿泊旅行統計調査（年確定値）」の原Excelを、都道府県・年月・公表区分単位へ正規化する。入力は第1表、第4表、第8表の「全体」列であり、施設タイプ別内訳は対象外とする。
...
| `release_type` | TEXT | 現在採用する都道府県データでは`final` |
...
|---|---|---|
| `year`, `month` | INTEGER | 調査対象年月 |
| `release_type` | TEXT | 現在採用する全国データでは`final` |
...
| View | 内容 |
|---|---|
| `latest_prefecture_market` | 同一年月・都道府県について、確定値、第2次速報、第1次速報の順で都道府県月次ファクトを選択し、都道府県名を付与 |
